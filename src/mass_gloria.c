...
void mass_gloria() {
    ...
    c2html_init("../prayer/mass/gloria", .css_path = "../../style.css", .js_path = "../common.js", .title = "Glória");


    go_back_button();

    with_tag(h1) { add_text("Glória"); }
    ...
    with_tag(div, .id = "latin", .css_class = "left-justified") {
        ...
        TextLines text_la = read_file("prayer_text/gloria_LA.txt");
    ...
    with_tag(div, .id = "portuguese", .css_class = "left-justified") {
        ...
        TextLines text_br = read_file("prayer_text/gloria_BR.txt");
