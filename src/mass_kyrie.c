#include "common.h"

void mass_kyrie() {

    c2html_init("../prayer/mass/kyrie", .css_path = "../../style.css", .js_path = "../common.js", .title = "Kyrie");


    go_back_button();

    with_tag(h1) { add_text("Kýrie"); }

    with_tag(hr, .no_close = true) {}

    with_tag(div, .id = "latin", .css_class = "left-justified") {
        with_tag(h1) { add_text("Grego"); }
        br();

        TextLines text_la = read_file("prayer_text/kyrie_LA.txt");

        for(int i = 0; i < text_la.count; ++i) {
            add_text(text_la.lines[i]);
            br_repeat(2);
        }
    }

    with_tag(hr, .no_close = true);

    with_tag(div, .id = "portuguese", .css_class = "left-justified") {

        with_tag(h1) { add_text("Português"); }
        br();

        TextLines text_br = read_file("prayer_text/kyrie_BR.txt");

        for(int i = 0; i < text_br.count; ++i) {
            add_text(text_br.lines[i]);
            br_repeat(2);
        }

    }

    c2html_end_file();

}
